...
fn bm_test(bm: &[u8], ndx: usize) -> u8 {
    ...
    unsafe {
        if (*bm.get_unchecked(ndx >> 3) & (1 << (ndx & 0x07))) != 0 { 1 } else { 0 }
    }
}
...
#[inline(always)]
fn is_a(seq: &[u8], n: usize) -> bool { nuc_val(seq, n) == 0 }

#[inline(always)]
fn is_c(seq: &[u8], n: usize) -> bool { nuc_val(seq, n) == 2 }

#[inline(always)]
fn is_g(seq: &[u8], n: usize) -> bool { nuc_val(seq, n) == 1 }

#[inline(always)]
fn is_t(seq: &[u8], n: usize) -> bool { nuc_val(seq, n) == 3 }
...
fn build_codon_table(tt: i32) -> [u8; 64] {
    ...
    for val in 0u8..64 {
        ...
        let stop = match (n0, n1, n2) {
            ...
            (3, 1, 0) => !((tt >= 2 && tt <= 5) || tt == 9 || tt == 10
                          || tt == 13 || tt == 14 || tt == 21 || tt == 25), // TGA
        ...
        if stop { table[val as usize] |= CC_STOP; continue; }

        // --- Start codons ---
        // ATG: universal start
        if n0 == 0 && n1 == 3 && n2 == 1 { // ATG
...
fn is_stop(seq: &[u8], n: usize, tt: i32) -> bool {
    ...
    if is_t(seq, n) && is_a(seq, n + 1) && is_g(seq, n + 2) {
        if tt == 6 || tt == 15 || tt == 16 || tt == 22 { return false; }
        return true;
    }
    // TGA
    if is_t(seq, n) && is_g(seq, n + 1) && is_a(seq, n + 2) {
        if (tt >= 2 && tt <= 5) || tt == 9 || tt == 10 || tt == 13 || tt == 14
            || tt == 21 || tt == 25
        {
            return false;
        }
    ...
    if is_t(seq, n) && is_a(seq, n + 1) && is_a(seq, n + 2) {
        if tt == 6 || tt == 14 { return false; }
        return true;
    }
    // Code 2
    if tt == 2 && is_a(seq, n) && is_g(seq, n + 1) && is_a(seq, n + 2) { return true; }
    if tt == 2 && is_a(seq, n) && is_g(seq, n + 1) && is_g(seq, n + 2) { return true; }
    // Code 22
    if tt == 22 && is_t(seq, n) && is_c(seq, n + 1) && is_a(seq, n + 2) { return true; }
    // Code 23
    if tt == 23 && is_t(seq, n) && is_t(seq, n + 1) && is_a(seq, n + 2) { return true; }
    false
}

fn is_start(seq: &[u8], n: usize, tt: i32) -> bool {
    // ATG
    if is_a(seq, n) && is_t(seq, n + 1) && is_g(seq, n + 2) { return true; }
    if tt == 6 || tt == 10 || tt == 14 || tt == 15 || tt == 16 || tt == 22 {
        return false;
    }
    // GTG
    if is_g(seq, n) && is_t(seq, n + 1) && is_g(seq, n + 2) {
        if tt == 1 || tt == 3 || tt == 12 || tt == 22 { return false; }
        return true;
    }
    // TTG
    if is_t(seq, n) && is_t(seq, n + 1) && is_g(seq, n + 2) {
        if tt < 4 || tt == 9 || (tt >= 21 && tt < 25) { return false; }
        return true;
    }
    false
}
...
fn rcom_seq(seq: &[u8], rseq: &mut [u8], useq: &[u8], len: usize) {
    ...
    for b in 0..full_bytes {
        ...
        unsafe { *rseq.get_unchecked_mut(b) = n0 | (n1 << 2) | (n2 << 4) | (n3 << 6); }
    }
    ...
    if remainder > 0 {
        ...
        unsafe { *rseq.get_unchecked_mut(full_bytes) = byte; }
    }
...
impl Default for Node {
    fn default() -> Self {
        Node {
            ntype: 0, edge: 0, ndx: 0, strand: 0, stop_val: 0,
            star_ptr: [0; 3], gc_bias: 0, gc_score: [0.0; 3],
            cscore: 0.0, gc_cont: 0.0, rbs: [0; 2], mot: Motif::default(),
            uscore: 0.0, tscore: 0.0, rscore: 0.0, sscore: 0.0,
            traceb: -1, tracef: -1, ov_mark: -1, score: 0.0, elim: 0,
        }
    }
}
...
impl SeqReader {
    fn new(path: &Path) -> io::Result<Self> {
        ...
        Ok(SeqReader { lines, pos: 0, sctr: 0, next_header: String::new() })
    }
    ...
    fn next_seq(
        ...
        while self.pos < self.lines.len() {
            ...
            if line.starts_with('>') || (line.starts_with("SQ")) ||
               (line.len() > 6 && &line[..6] == "ORIGIN")
            {
                ...
                for &ch in line.as_bytes() {
                    ...
                    match ch {
                        b'g' | b'G' => { bm_set(seq, bctr); gc_cont += 1; }
                        b't' | b'T' => { bm_set(seq, bctr); bm_set(seq, bctr + 1); }
                        b'c' | b'C' => { bm_set(seq, bctr + 1); gc_cont += 1; }
                        b'a' | b'A' => { /* A = 00, already cleared */ }
                        _ => { bm_set(seq, bctr + 1); bm_set(useq, len); } // N -> C encoding + useq mark
                    }
                    bctr += 2;
                    len += 1;
                }
...
fn imin(x: i32, y: i32) -> i32 { if x < y { x } else { y } }
fn dmin(x: f64, y: f64) -> f64 { if x < y { x } else { y } }
fn dmax(x: f64, y: f64) -> f64 { if x > y { x } else { y } }
...
fn shine_dalgarno_exact(seq: &[u8], pos: i32, start: i32, rwt: &[f64; 28]) -> i32 {
    ...
    for i in 0..limit {
        ...
        if pi >= 0 {
            let p = pi as usize;
            if i % 3 == 0 && is_a(seq, p) { match_arr[i] = 2.0; }
            else if i % 3 != 0 && is_g(seq, p) { match_arr[i] = 3.0; }
        }
    }
    ...
    while i >= 3 {
        for j in 0..=(limit - i) {
            ...
            for k in j..(j + i) {
                cur_ctr += match_arr[k];
                if match_arr[k] < 0.0 { mism += 1; }
            }
            if mism > 0 { continue; }
            let rdis = start - (pos + (j + i) as i32);
            let dis_flag: i32;
            if rdis < 5 && (i as i32) < 5 { dis_flag = 2; }
            else if rdis < 5 && (i as i32) >= 5 { dis_flag = 1; }
            else if rdis > 10 && rdis <= 12 && (i as i32) < 5 { dis_flag = 1; }
            else if rdis > 10 && rdis <= 12 && (i as i32) >= 5 { dis_flag = 2; }
            ...
            if rwt[cur_val as usize] < rwt[max_val as usize] { continue; }
            if rwt[cur_val as usize] == rwt[max_val as usize] && cur_val < max_val { continue; }
            max_val = cur_val;
        }
        if i == 0 { break; }
        i -= 1;
...
fn shine_dalgarno_mm(seq: &[u8], pos: i32, start: i32, rwt: &[f64; 28]) -> i32 {
    ...
    for i in 0..limit {
        ...
        if pi >= 0 {
            let p = pi as usize;
            if i % 3 == 0 {
                if is_a(seq, p) { match_arr[i] = 2.0; }
                else { match_arr[i] = -3.0; }
            } else {
                if is_g(seq, p) { match_arr[i] = 3.0; }
                else { match_arr[i] = -2.0; }
            }
        }
    }
...
fn add_nodes(
    seq: &[u8], rseq: &[u8], slen: usize,
    nodes: &mut Vec<Node>, closed: bool,
    mlist: &[Mask], tt: i32,
) -> usize {
    nodes.clear();
    ...
    while i >= 0 {
        ...
        if cc & CC_STOP != 0 {
            if saw_start[fr] {
                ...
                if ct[codon_val(seq, last[fr] as usize) as usize] & CC_STOP == 0 { n.edge = 1; }
                n.ndx = last[fr];
                n.ntype = STOP;
                n.strand = 1;
                n.stop_val = i;
        ...
        if last[fr] >= slen_i { i -= 1; continue; }

        let gene_len = last[fr] - i + 3;
        if cc & CC_ATG != 0 && gene_len >= min_dist[fr]
            && !cross_mask(i, last[fr], mlist)
        {
            let mut n = Node::default();
            n.ndx = i; n.ntype = ATG; saw_start[fr] = true;
            n.stop_val = last[fr]; n.strand = 1;
            nodes.push(n);
        } else if cc & CC_GTG != 0 && gene_len >= min_dist[fr]
            && !cross_mask(i, last[fr], mlist)
        {
            let mut n = Node::default();
            n.ndx = i; n.ntype = GTG; saw_start[fr] = true;
            n.stop_val = last[fr]; n.strand = 1;
            nodes.push(n);
        } else if cc & CC_TTG != 0 && gene_len >= min_dist[fr]
            && !cross_mask(i, last[fr], mlist)
        {
            let mut n = Node::default();
            n.ndx = i; n.ntype = TTG; saw_start[fr] = true;
            n.stop_val = last[fr]; n.strand = 1;
            nodes.push(n);
            ...
            && !cross_mask(i, last[fr], mlist)
        {
            let mut n = Node::default();
            n.ndx = i; n.ntype = ATG; saw_start[fr] = true;
            n.edge = 1; n.stop_val = last[fr]; n.strand = 1;
            nodes.push(n);
        }
        i -= 1;
    }
    for i in 0..3 {
        if saw_start[i] {
            let mut n = Node::default();
            if ct[codon_val(seq, last[i] as usize) as usize] & CC_STOP == 0 { n.edge = 1; }
            n.ndx = last[i];
            n.ntype = STOP;
            n.strand = 1;
    ...
    while i >= 0 {
        ...
        if cc & CC_STOP != 0 {
            if saw_start[fr] {
                let mut n = Node::default();
                if ct[codon_val(rseq, last[fr] as usize) as usize] & CC_STOP == 0 { n.edge = 1; }
        ...
        {
            let mut n = Node::default();
            n.ndx = slen_i - i - 1; n.ntype = ATG; saw_start[fr] = true;
            n.stop_val = slen_i - last[fr] - 1; n.strand = -1;
            nodes.push(n);
        } else if cc & CC_GTG != 0 && gene_len >= min_dist[fr]
            && !cross_mask(slen_i - last[fr] - 1, slen_i - i - 1, mlist)
        {
            let mut n = Node::default();
            n.ndx = slen_i - i - 1; n.ntype = GTG; saw_start[fr] = true;
            n.stop_val = slen_i - last[fr] - 1; n.strand = -1;
            nodes.push(n);
        } else if cc & CC_TTG != 0 && gene_len >= min_dist[fr]
            && !cross_mask(slen_i - last[fr] - 1, slen_i - i - 1, mlist)
        {
            let mut n = Node::default();
            n.ndx = slen_i - i - 1; n.ntype = TTG; saw_start[fr] = true;
            n.stop_val = slen_i - last[fr] - 1; n.strand = -1;
            nodes.push(n);
        } else if i <= 2 && !closed && (last[fr] - i) > MIN_EDGE_GENE
            && !cross_mask(slen_i - last[fr] - 1, slen_i - i - 1, mlist)
        {
            let mut n = Node::default();
            n.ndx = slen_i - i - 1; n.ntype = ATG; saw_start[fr] = true;
            n.edge = 1; n.stop_val = slen_i - last[fr] - 1; n.strand = -1;
            nodes.push(n);
        }
        i -= 1;
    }
    for i in 0..3 {
        if saw_start[i] {
            let mut n = Node::default();
            if !is_stop(rseq, last[i] as usize, tt) { n.edge = 1; }
    ...
    nodes.sort_by(|a, b| {
        a.ndx.cmp(&b.ndx).then_with(|| b.strand.cmp(&a.strand))
    });
...
fn reset_node_scores(nodes: &mut [Node]) {
    for n in nodes.iter_mut() {
        ...
        n.gc_score = [0.0; 3];
        n.rbs = [0; 2];
        n.score = 0.0; n.cscore = 0.0; n.sscore = 0.0;
        n.rscore = 0.0; n.tscore = 0.0; n.uscore = 0.0;
        n.traceb = -1; n.tracef = -1; n.ov_mark = -1;
        n.elim = 0; n.gc_bias = 0;
        n.mot = Motif::default();
    }
}
...
fn record_overlapping_starts(nodes: &mut [Node], tinf: &Training, flag: i32) {
    ...
    for i in 0..nn {
        ...
        if nodes[i].ntype != STOP || nodes[i].edge == 1 { continue; }
        if nodes[i].strand == 1 {
            ...
            while j >= 0 {
                let ju = j as usize;
                if ju >= nn || nodes[ju].ndx > nodes[i].ndx + 2 { j -= 1; continue; }
                if nodes[ju].ndx + MAX_SAM_OVLP < nodes[i].ndx { break; }
                if nodes[ju].strand == 1 && nodes[ju].ntype != STOP {
                    if nodes[ju].stop_val <= nodes[i].ndx { j -= 1; continue; }
                    let fr = (nodes[ju].ndx % 3) as usize;
                    if flag == 0 && nodes[i].star_ptr[fr] == -1 {
                        nodes[i].star_ptr[fr] = j;
                    } else if flag == 1 {
                        let sc = nodes[ju].cscore + nodes[ju].sscore
            ...
            let mut j = start_j;
            while j < nn {
                if nodes[j].ndx < nodes[i].ndx - 2 { j += 1; continue; }
                if nodes[j].ndx - MAX_SAM_OVLP > nodes[i].ndx { break; }
                if nodes[j].strand == -1 && nodes[j].ntype != STOP {
                    if nodes[j].stop_val >= nodes[i].ndx { j += 1; continue; }
                    let fr = (nodes[j].ndx % 3) as usize;
                    if flag == 0 && nodes[i].star_ptr[fr] == -1 {
                        nodes[i].star_ptr[fr] = j as i32;
                    } else if flag == 1 {
                        let sc = nodes[j].cscore + nodes[j].sscore
...
fn intergenic_mod(n1: &Node, n2: &Node, tinf: &Training) -> f64 {
    ...
    {
        if n1.strand == 1 && n2.rscore < 0.0 { rval -= n2.rscore; }
        if n1.strand == -1 && n1.rscore < 0.0 { rval -= n1.rscore; }
        if n1.strand == 1 && n2.uscore < 0.0 { rval -= n2.uscore; }
        if n1.strand == -1 && n1.uscore < 0.0 { rval -= n1.uscore; }
    }
    let dist = (n1.ndx - n2.ndx).abs();
    if n1.strand == 1 && n2.strand == 1 && n1.ndx + 2 >= n2.ndx { ovlp = 1.0; }
    else if n1.strand == -1 && n2.strand == -1 && n1.ndx >= n2.ndx + 2 { ovlp = 1.0; }
...
fn record_gc_bias(gc: &[i32], nodes: &mut [Node], tinf: &mut Training) {
    ...
    if tot > 0.0 {
        for i in 0..3 { tinf.bias[i] *= 3.0 / tot; }
    }
}
...
fn calc_orf_gc(seq: &[u8], _rseq: &[u8], _slen: usize, nodes: &mut [Node], _tinf: &Training) {
    ...
    for i in (0..nn).rev() {
        ...
        if nodes[i].strand == 1 && nodes[i].ntype == STOP {
            ...
            let mut j = last[fr] - 3;
            while j >= nodes[i].ndx {
                gc[fr] += (is_gc(seq, j as usize) + is_gc(seq, (j + 1) as usize)
...
fn raw_coding_score(seq: &[u8], rseq: &[u8], slen: usize, nodes: &mut [Node], tinf: &Training) {
    ...
    for i in 0..nn {
        let fr = (nodes[i].ndx % 3) as usize;
        if nodes[i].strand == 1 && nodes[i].ntype == STOP { score[fr] = -10000.0; }
        else if nodes[i].strand == 1 {
            if nodes[i].cscore > score[fr] { score[fr] = nodes[i].cscore; }
            else { nodes[i].cscore -= score[fr] - nodes[i].cscore; }
        }
    }
    score = [-10000.0; 3];
    for i in (0..nn).rev() {
    ...
    for i in 0..nn {
        ...
        if nodes[i].strand == 1 && nodes[i].ntype == STOP { score[fr] = -10000.0; }
            ...
            let mut lf = lfac;
            if lf > score[fr] { score[fr] = lf; }
            else { lf -= dmax(dmin(score[fr] - lf, lf), 0.0); }
            if lf > 3.0 && nodes[i].cscore < 0.5 * lf { nodes[i].cscore = 0.5 * lf; }
            nodes[i].cscore += lf;
        }
    }
    for i in (0..nn).rev() {
    ...
    for i in 0..nn {
        ...
        if nodes[i].strand == 1 {
            for j in (nodes[i].ndx - 20)..=(nodes[i].ndx - 6) {
                ...
                if e > nodes[i].rbs[0] { nodes[i].rbs[0] = e; }
                if m > nodes[i].rbs[1] { nodes[i].rbs[1] = m; }
            }
        } else {
            let slen_i = slen as i32;
            for j in (slen_i - nodes[i].ndx - 21)..=(slen_i - nodes[i].ndx - 7) {
                if j > slen_i - 1 { continue; }
...
fn score_upstream_composition(seq: &[u8], slen: usize, nod: &mut Node, tinf: &Training) {
    let start = if nod.strand == 1 { nod.ndx } else { slen as i32 - 1 - nod.ndx };
    nod.uscore = 0.0;
    ...
    for i in 1..45i32 {
        ...
        if start - i >= 0 {
            nod.uscore += 0.4 * tinf.st_wt
                * tinf.ups_comp[count][mer_ndx(1, seq, (start - i) as usize)];
        }
        count += 1;
    }
}
...
fn find_best_upstream_motif(tinf: &Training, seq: &[u8], rseq: &[u8], slen: usize, nod: &mut Node, stage: i32) {
    if nod.ntype == STOP || nod.edge == 1 { return; }
    ...
    if stage == 2 && (max_sc == -4.0 || max_sc < tinf.no_mot + 0.69) {
        nod.mot = Motif { ndx: 0, len: 0, spacendx: 0, spacer: 0, score: tinf.no_mot };
    } else {
        nod.mot = Motif { ndx: max_ndx, len: max_len, spacendx: max_spacendx, spacer: max_spacer, score: max_sc };
    }
}
...
fn score_nodes(
    seq: &[u8], rseq: &[u8], slen: usize,
    nodes: &mut [Node], tinf: &Training,
    closed: bool, is_meta: bool,
) {
    let nn = nodes.len();
    let slen_i = slen as i32;
    ...
    for i in 0..nn {
        if nodes[i].ntype == STOP { continue; }

        let mut edge_gene = 0.0f64;
        if nodes[i].edge == 1 { edge_gene += 1.0; }
        if (nodes[i].strand == 1 && !is_stop(seq, nodes[i].stop_val as usize, tinf.trans_table))
            || (nodes[i].strand == -1
                && !is_stop(rseq, (slen_i - 1 - nodes[i].stop_val) as usize, tinf.trans_table))
        {
            edge_gene += 1.0;
        }
        ...
        if edge_gene == 0.0 && gene_len < 250 {
            ...
            if nodes[i].rscore < 0.0 { nodes[i].rscore *= negf; }
            if nodes[i].uscore < 0.0 { nodes[i].uscore *= negf; }
            if nodes[i].tscore < 0.0 { nodes[i].tscore *= negf; }
            if nodes[i].rscore > 0.0 { nodes[i].rscore *= posf; }
            if nodes[i].uscore > 0.0 { nodes[i].uscore *= posf; }
            if nodes[i].tscore > 0.0 { nodes[i].tscore *= posf; }
        }

        // Meta penalization (not used in single mode but included for correctness)
        if is_meta && slen < 3000 && edge_gene == 0.0
            && (nodes[i].cscore < 5.0 || gene_len < 120)
        {
        ...
        if nodes[i].cscore < 0.0 {
            if edge_gene > 0.0 && nodes[i].edge == 0 {
                ...
                if gene_len as f64 >= min_meta_len {
                    if nodes[i].cscore >= 0.0 { nodes[i].cscore = -1.0; }
                    nodes[i].sscore = 0.0;
                    nodes[i].uscore = 0.0;
                }
            } else {
...
fn dprog(nodes: &mut [Node], tinf: &Training, flag: i32) -> i32 {
    ...
    let cats: Vec<u8> = nodes.iter().map(|n| {
        match (n.strand, n.ntype) {
            ( 1, STOP) => 1,
            ( 1, _)    => 0,
            (-1, STOP) => 2,
            _          => 3,
    ...
    for i in 0..nn {
        let mut min = if i < MAX_NODE_DIST { 0 } else { i - MAX_NODE_DIST };
        if nodes[i].strand == -1 && nodes[i].ntype != STOP
            && nodes[min].ndx >= nodes[i].stop_val
        {
            while min > 0 && nodes[min].ndx >= nodes[i].stop_val { min -= 1; }
        }
        if nodes[i].strand == 1 && nodes[i].ntype == STOP
            && nodes[min].ndx >= nodes[i].stop_val
        {
        ...
        if min < MAX_NODE_DIST { min = 0; } else { min -= MAX_NODE_DIST; }
        ...
        for j in min..i {
            ...
            if !VALID[cat_j as usize][cat_i as usize] { continue; }
            if unsafe { !*alive.get_unchecked(j) } { continue; }
            score_connection(nodes, j, i, tinf, flag);
        }
        // Update alive status for node i after it may have received a connection
        let cat = unsafe { *cats.get_unchecked(i) };
        if cat == 1 || cat == 3 {
            unsafe { *alive.get_unchecked_mut(i) = nodes.get_unchecked(i).traceb != -1; }
        }
    }
    ...
    for i in (0..nn).rev() {
        if nodes[i].strand == 1 && nodes[i].ntype != STOP { continue; }
        if nodes[i].strand == -1 && nodes[i].ntype == STOP { continue; }
        if nodes[i].score > max_sc { max_sc = nodes[i].score; max_ndx = i as i32; }
    }

    if max_ndx < 0 { return -1; }
    ...
    while nodes[path as usize].traceb != -1 {
        ...
        if nodes[path as usize].strand == -1 && nodes[path as usize].ntype == STOP
            && nodes[nxt as usize].strand == 1 && nodes[nxt as usize].ntype == STOP
            && nodes[path as usize].ov_mark != -1 && nodes[path as usize].ndx > nodes[nxt as usize].ndx
        {
            let tmp = nodes[path as usize].star_ptr[nodes[path as usize].ov_mark as usize];
            let mut ii = tmp;
            while nodes[ii as usize].ndx != nodes[tmp as usize].stop_val { ii -= 1; }
    ...
    while nodes[path as usize].traceb != -1 {
        ...
        {
            let mut ii = path;
            while nodes[ii as usize].ndx != nodes[path as usize].stop_val { ii -= 1; }
    ...
    if nodes[max_ndx as usize].traceb == -1 { -1 } else { max_ndx }
}
...
fn score_connection(nodes: &mut [Node], p1: usize, p2: usize, tinf: &Training, flag: i32) {
    ...
    if n1_strand == n2_strand && n1_strand == 1 && n1_type != STOP && n2_type == STOP {
        if n2_stop >= n1_ndx { return; }
        if n1_ndx % 3 != n2_ndx % 3 { return; }
        right += 2;
        if flag == 0 {
    ...
    // 3'fwd->3'rev
        ...
        for fr in 0..3 {
            ...
            if o <= 0 || o >= MAX_OPP_OVLP { continue; }
            if o >= nodes[sp_u].ndx - left { continue; }
            if n1_traceb == -1 { continue; }
            if o >= nodes[sp_u].stop_val - nodes[n1_traceb as usize].ndx - 2 { continue; }
            if flag == 1 {
                let s = nodes[sp_u].cscore + nodes[sp_u].sscore
        ...
        if maxfr != -1 {
            ...
            if flag == 0 {
                ...
                score = nodes[sp].cscore + nodes[sp].sscore
    ...
    // 5'rev->3'rev
        ...
        right -= 2;
        if left >= right { return; }
        if flag == 1 { score = intergenic_mod(&nodes[p1], &nodes[p2], tinf); }
    }
    // 5'rev->5'fwd
    else if n1_strand == -1 && n1_type != STOP && n2_strand == 1 && n2_type != STOP {
        if left >= right { return; }
    ...
    // 3'fwd->3'fwd (operon)
        ...
        if nodes[p1].star_ptr[sp_idx] == -1 { return; }
        ...
        if flag == 0 {
            ...
            score = nodes[sp].cscore + nodes[sp].sscore
                + intergenic_mod(&nodes[p1], &nodes[sp], tinf);
        }
    }
    // 3'rev->3'rev (operon)
    else if n1_strand == -1 && n1_type == STOP && n2_strand == -1 && n2_type == STOP {
        ...
        if nodes[p2].star_ptr[sp_idx] == -1 { return; }
    ...
    // 3'fwd->5'rev (opposite strand overlap)
        ...
        if n2_stop - 2 >= n1_ndx + 2 { return; }
        ovlp = (n1_ndx + 2) - (n2_stop - 2) + 1;
        if ovlp >= MAX_OPP_OVLP { return; }
        if (n1_ndx + 2 - n2_stop - 2 + 1) >= (n2_ndx - n1_ndx + 3 + 1) { return; }
        let bnd = if n1_traceb == -1 { 0 } else { nodes[n1_traceb as usize].ndx };
        if (n1_ndx + 2 - n2_stop - 2 + 1) >= (n2_stop - 3 - bnd + 1) { return; }
        left = n2_stop - 2;
        if flag == 0 {
            scr_mod = tinf.bias[0] * nodes[p2].gc_score[0]
                + tinf.bias[1] * nodes[p2].gc_score[1]
...
fn eliminate_bad_genes(nodes: &mut [Node], dbeg: i32, tinf: &Training) {
    ...
    // Find start of path
    let mut path = dbeg;
    while nodes[path as usize].traceb != -1 { path = nodes[path as usize].traceb; }
...
fn add_genes(genes: &mut Vec<Gene>, nodes: &[Node], dbeg: i32) -> usize {
    ...
    while path != -1 {
        ...
        if nodes[pu].strand == 1 && nodes[pu].ntype != STOP {
            if ctr >= genes.len() { genes.push(Gene::default()); }
...
fn tweak_final_starts(genes: &mut [Gene], nodes: &mut [Node], tinf: &Training) {
    ...
    for i in 0..ng {
        ...
        for j in lo..hi {
            if j == ndx { continue; }
            if nodes[j].ntype == STOP || nodes[j].stop_val != nodes[ndx].stop_val { continue; }

            let mut tigm = 0.0f64;
            if i > 0 && nodes[j].strand == 1 && nodes[genes[i - 1].start_ndx].strand == 1 {
                if nodes[genes[i - 1].stop_ndx].ndx - nodes[j].ndx > MAX_SAM_OVLP { continue; }
                tigm = intergenic_mod(&nodes[genes[i - 1].stop_ndx], &nodes[j], tinf);
            }
            if i > 0 && nodes[j].strand == 1 && nodes[genes[i - 1].start_ndx].strand == -1 {
                if nodes[genes[i - 1].start_ndx].ndx - nodes[j].ndx >= 0 { continue; }
                tigm = intergenic_mod(&nodes[genes[i - 1].start_ndx], &nodes[j], tinf);
            }
            if i < ng - 1 && nodes[j].strand == -1 && nodes[genes[i + 1].start_ndx].strand == 1 {
                if nodes[j].ndx - nodes[genes[i + 1].start_ndx].ndx >= 0 { continue; }
                tigm = intergenic_mod(&nodes[j], &nodes[genes[i + 1].start_ndx], tinf);
            }
            if i < ng - 1 && nodes[j].strand == -1 && nodes[genes[i + 1].start_ndx].strand == -1 {
                if nodes[j].ndx - nodes[genes[i + 1].stop_ndx].ndx > MAX_SAM_OVLP { continue; }
            ...
            if maxndx[0] == -1 {
                ...
                maxndx[1] = maxndx[0]; maxsc[1] = maxsc[0]; maxigm[1] = maxigm[0];
                maxndx[0] = j as i32; maxsc[0] = jsc; maxigm[0] = tigm;
            } else if maxndx[1] == -1 || jsc + tigm > maxsc[1] {
                maxndx[1] = j as i32; maxsc[1] = jsc; maxigm[1] = tigm;
            }
        }
        ...
        for j in 0..2 {
            ...
            // Close starts
                ...
                && nodes[ndx].edge == 0 && nodes[mu].edge == 0
            {
        ...
        for j in 0..2 {
            ...
            if best == -1 && maxsc[j] + maxigm[j] > sc + igm {
                best = j as i32;
            } else if best >= 0 && maxsc[j] + maxigm[j] > maxsc[best as usize] + maxigm[best as usize] {
                best = j as i32;
            }
        }
...
fn build_gene_data(
    gene: &Gene, nodes: &[Node], tinf: &Training,
    gene_num: usize, num_seq: i32,
) -> String {
    ...
    let partial_left = if (nodes[ndx].edge == 1 && nodes[ndx].strand == 1) ||
                          (nodes[sndx].edge == 1 && nodes[ndx].strand == -1) { 1 } else { 0 };
    let partial_right = if (nodes[sndx].edge == 1 && nodes[ndx].strand == 1) ||
                           (nodes[ndx].edge == 1 && nodes[ndx].strand == -1) { 1 } else { 0 };

    // start type
    let st_type = if nodes[ndx].edge == 1 { 3usize } else { nodes[ndx].ntype as usize };
    ...
    if tinf.uses_sd == 1 {
        if rbs1 > rbs2 {
            data.push_str(&format!("rbs_motif={};rbs_spacer={}",
                SD_STRING[nodes[ndx].rbs[0] as usize],
                SD_SPACER[nodes[ndx].rbs[0] as usize]));
        } else {
            data.push_str(&format!("rbs_motif={};rbs_spacer={}",
                SD_STRING[nodes[ndx].rbs[1] as usize],
                SD_SPACER[nodes[ndx].rbs[1] as usize]));
        }
    } else {
        ...
        if tinf.no_mot > -0.5 && rbs1 > rbs2 && rbs1 > nodes[ndx].mot.score * tinf.st_wt {
            ...
            data.push_str(&format!("rbs_motif={};rbs_spacer={}bp", qt, nodes[ndx].mot.spacer));
        }
    }
...
fn write_nucleotide_seqs(
    out: &mut Vec<u8>,
    genes: &[Gene], nodes: &[Node],
    seq: &[u8], rseq: &[u8], useq: &[u8],
    slen: usize, tinf: &Training,
    num_seq: i32, short_header: &str,
) {
    for (i, gene) in genes.iter().enumerate() {
        ...
        let hdr = format!(
            ">{}_{} # {} # {} # {} # {}\n",
            short_header, i + 1, gene.begin, gene.end, strand_str, gene_data,
        );
        ...
        if nodes[ndx].strand == 1 {
            ...
            for j in (gene.begin - 1)..gene.end {
                let ju = j as usize;
                if is_a(seq, ju) { out.push(b'A'); }
                else if is_t(seq, ju) { out.push(b'T'); }
                else if is_g(seq, ju) { out.push(b'G'); }
                else if is_c(seq, ju) && !is_n(useq, ju) { out.push(b'C'); }
                else { out.push(b'N'); }
                col += 1;
                if col % 70 == 0 { out.push(b'\n'); }
            }
            if col % 70 != 0 { out.push(b'\n'); }
        } else {
            let slen_i = slen as i32;
            ...
            for j in (slen_i - gene.end)..(slen_i + 1 - gene.begin) {
                let ju = j as usize;
                if is_a(rseq, ju) { out.push(b'A'); }
                else if is_t(rseq, ju) { out.push(b'T'); }
                else if is_g(rseq, ju) { out.push(b'G'); }
                else if is_c(rseq, ju) && !is_n(useq, slen - 1 - ju) { out.push(b'C'); }
                else { out.push(b'N'); }
                col += 1;
                if col % 70 == 0 { out.push(b'\n'); }
            }
...
fn run_prodigal_with_bufs(
    ...
    loop {
        ...
        let nn = add_nodes(&bufs.seq, &bufs.rseq, slen, &mut bufs.nodes, closed, &masks, trans_table);
        if nn == 0 { continue; }

        reset_node_scores(&mut bufs.nodes);
        score_nodes(&bufs.seq, &bufs.rseq, slen, &mut bufs.nodes, training, closed, false);
        ...
        write_nucleotide_seqs(
            &mut bufs.output, &bufs.genes, &bufs.nodes,
            &bufs.seq, &bufs.rseq, &bufs.useq,
            slen, training, num_seq, &short_header,
        );
    }
...
mod tests {
    ...
    fn test_vs_c_prodigal() {
        ...
        let genome_str = match std::env::var("PRODIGAL_TEST_GENOME") {
            Ok(v) => v,
            Err(_) => { eprintln!("PRODIGAL_TEST_GENOME not set, skipping test"); return; }
        };
        let trn_str = match std::env::var("PRODIGAL_TEST_TRN") {
            Ok(v) => v,
            Err(_) => { eprintln!("PRODIGAL_TEST_TRN not set, skipping test"); return; }
        };
        ...
        let c_result = Command::new(&prodigal_bin)
            .args(&["-i", genome.to_str().unwrap(), "-t", trn_path.to_str().unwrap(),
                    "-g", "11", "-d", nuc_file, "-f", "sco", "-q"])
            .output()
        ...
        struct COutput { stdout: Vec<u8> }
        let c_output = COutput { stdout: c_stdout };

        // Run Rust prodigal
        let training = Training::from_file(trn_path).expect("Failed to load training file");
        let rs_output = run_prodigal(genome, &training, 11, false)
            .expect("Rust prodigal failed");

        // Compare headers first
        let c_headers: Vec<&str> = std::str::from_utf8(&c_output.stdout).unwrap()
            .lines().filter(|l| l.starts_with('>')).collect();
        let rs_headers: Vec<&str> = std::str::from_utf8(&rs_output).unwrap()
            .lines().filter(|l| l.starts_with('>')).collect();

        eprintln!("C prodigal: {} genes, {} bytes", c_headers.len(), c_output.stdout.len());
        eprintln!("Rust prodigal: {} genes, {} bytes", rs_headers.len(), rs_output.len());
        ...
        if c_output.stdout != rs_output {
            ...
            for i in 0..min_len {
                if c_output.stdout[i] != rs_output[i] {
                    ...
                    eprintln!("  C:  {:?}", String::from_utf8_lossy(&c_output.stdout[start..end]));
                    eprintln!("  RS: {:?}", String::from_utf8_lossy(&rs_output[start..end]));
                    break;
                }
            }
            if c_output.stdout.len() != rs_output.len() {
                eprintln!("Output length differs: C={}, RS={}", c_output.stdout.len(), rs_output.len());
            }
        }

        assert_eq!(c_output.stdout, rs_output, "Output does not match C prodigal");
    }
    ...
    fn test_vs_c_prodigal_multi_genomes() {
        ...
        let genomes_str = match std::env::var("PRODIGAL_TEST_GENOMES_DIR") {
            Ok(v) => v,
            Err(_) => { eprintln!("PRODIGAL_TEST_GENOMES_DIR not set, skipping test"); return; }
        };
        let trn_str = match std::env::var("PRODIGAL_TEST_TRN") {
            Ok(v) => v,
            Err(_) => { eprintln!("PRODIGAL_TEST_TRN not set, skipping test"); return; }
        ...
        let mut entries: Vec<_> = std::fs::read_dir(genomes_dir).unwrap()
            .filter_map(|e| e.ok())
        ...
        for entry in &entries {
            ...
            let nuc_file = format!("/tmp/prodigal_rs_test_{}.fasta", genome.file_stem().unwrap().to_str().unwrap());
            ...
            if c_output == rs_output {
                ...
                total_fail += 1;
                eprintln!("MISMATCH: {} (C={} bytes, RS={} bytes)", genome.display(), c_output.len(), rs_output.len());
                // Find first byte difference
                ...
                for bi in 0..min_len {
                    if c_output[bi] != rs_output[bi] {
                        ...
                        eprintln!("    C:  {:?}", String::from_utf8_lossy(&c_output[start..end]));
                        eprintln!("    RS: {:?}", String::from_utf8_lossy(&rs_output[start..end]));
                        break;
                    }
                }
        ...
        eprintln!("{}/{} genomes matched perfectly", total_match, total_match + total_fail);
