...
fn main() {
    let genome = std::env::args()
        .nth(1)
        .expect("Usage: compare_cds <genome.fasta> <training.trn>");
    let trn = std::env::args()
        .nth(2)
        .expect("Usage: compare_cds <genome.fasta> <training.trn>");
    ...
    let training =
        chewcall::prodigal_rs::Training::from_file(trn_path).expect("Failed to load training");
