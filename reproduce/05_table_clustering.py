...
def main():
    ...
    for ds in DATASETS:
        for org in ORGANISMS:
            cc = DIR_CC / f"shcds_{ds}_{org}_rep1" / "results_alleles_hashed.tsv"
            cb_base = DIR_C354 / f"shcds_{ds}_{org}_rep1"
            cb = cb_base / "results_alleles_hashed.tsv"
            if not cb.exists():
                cb = next(cb_base.glob("results_*/results_alleles_hashed.tsv"), cb)
