...
run_one() {
    local label="$1" input="$2" schema_src="$3" outdir="$4" cdsflag="${5:-}"
    ...
    # In shared-CDS mode, $cdsflag is "--cds" so chewBBACA skips gene prediction
    # and treats the input FASTA files as pre-computed coding sequences. Without
    # it, chewBBACA re-runs Pyrodigal on the CDS files, producing a different CDS
    # set and spurious INF/EXC divergence against chewcall's shared-CDS run.
    conda run -n "$CONDA_ENV" chewie AlleleCall \
        -i "$input" -g "$schema_tmp" -o "$outdir" \
        --cpu "$CPU" $cdsflag --no-inferred --hash-profile crc32 \
        > "$outdir/run.log" 2>&1
    local END=$(date +%s.%N)
...
run_block() {
    ...
    for org in lm se ec cj; do
        for rep in $(seq 1 "$REPS"); do
            ...
            local cdsflag=""
            [ "$mode" = "shcds" ] && { input="${CDSDIR[$org]}"; cdsflag="--cds"; }
            run_one "$name" "$input" "${SCHEMA[$org]}" "$OUT_BASE/$name" "$cdsflag"
        done
    done
}
